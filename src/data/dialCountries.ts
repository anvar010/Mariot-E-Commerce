/**
 * Every country, with its ISO code and international dialling code.
 *
 * The GCC is listed first because that is where the customers are, so a Dubai clerk finds
 * the UAE without scrolling or typing. Everything after that block is alphabetical.
 *
 * Several countries share a dialling code -- +1 across North America and the Caribbean,
 * +7 across Russia and Kazakhstan, +39 for Italy and the Vatican. That is fine for a
 * picker, where the person chooses a country; it only matters when reading a code back
 * out of a number, which is why that lookup returns the longest match rather than a
 * country.
 */
...
export interface DialCountry {
    ...
    code: string;
    /** International dialling code, with its leading +. */
    dial: string;
    name: string;
}

const GCC: DialCountry[] = [
...
export const DIAL_COUNTRIES: DialCountry[] = [...GCC, ...REST];

export const DEFAULT_DIAL_COUNTRY: DialCountry = GCC[0];

/**
 * The country whose dialling code a number starts with, longest code first.
 *
 * Longest-first is required, not a preference: +1 is the US but +1242 is the Bahamas, and
 * +97 is nothing while +971 is the UAE. A shortest match would swallow the longer codes.
 *
 * Where several countries share a code the first listed wins, which puts the GCC ahead of
 * everything else -- the right default for this business.
 */
export const matchDialCountry = (value: string): DialCountry | null => {
