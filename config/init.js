...
const initDb = async () => {
    try {
        ...
        // 4a. Order lines have to outlive the products they refer to.
        //
        // order_items stored only product_id, and its foreign key was ON DELETE CASCADE,
        // so deleting a product deleted its line from every order ever placed. The order
        // kept its total while losing what was in it: the figures no longer reconciled,
        // and the invoice could not be regenerated. An order is a financial record and
        // has to stay readable whatever later happens to the catalogue.
        //
        // Two changes: the name and model are copied onto the line when the order is
        // placed, and the key becomes SET NULL so the line survives the product. Deleting
        // a product is still allowed -- it just no longer rewrites history.
        try {
            const [cols] = await db.query('SHOW COLUMNS FROM order_items');
            const names = cols.map(c => c.Field);

            for (const col of [
                { name: 'product_name', definition: 'VARCHAR(255) NULL AFTER product_id' },
                { name: 'product_model', definition: 'VARCHAR(120) NULL AFTER product_name' },
            ]) {
                if (!names.includes(col.name)) {
                    await db.query(`ALTER TABLE order_items ADD COLUMN ${col.name} ${col.definition}`);
                    console.log(`[DB] Migration: Added ${col.name} to order_items`);
                }
            }

            // Backfill from the products still present. Lines whose product is already
            // gone cannot be recovered -- there is nothing left to read the name from.
            const [filled] = await db.query(`
                UPDATE order_items oi
                JOIN products p ON p.id = oi.product_id
                SET oi.product_name = p.name,
                    oi.product_model = COALESCE(oi.product_model, p.model)
                WHERE oi.product_name IS NULL
            `);
            if (filled.affectedRows) {
                console.log(`[DB] Migration: named ${filled.affectedRows} historic order lines`);
            }

            // The constraint name is generated by MySQL, so it is looked up rather than
            // guessed. Only the products key is touched; the one to orders is correct as it
            // stands -- deleting an order should take its lines with it.
            ...
            `);
            const cascading = fks.filter(fk => String(fk.DELETE_RULE).toUpperCase() === 'CASCADE');

            for (const fk of cascading) {
                // Order matters here. MySQL refuses to change the nullability of a column
                // an active foreign key sits on, so the key comes off first, the column is
                // widened, and the key goes back on with the new rule.
                //
                // The window between dropping and re-adding is the one moment order_items
                // has no key to products, so the re-add is in a finally: if widening the
                // column fails, the original key is restored rather than left off.
                await db.query(`ALTER TABLE order_items DROP FOREIGN KEY \`${fk.CONSTRAINT_NAME}\``);
                let widened = false;
                try {
                    const productIdCol = cols.find(c => c.Field === 'product_id');
                    if (productIdCol && String(productIdCol.Null).toUpperCase() === 'NO') {
                        await db.query('ALTER TABLE order_items MODIFY product_id INT NULL');
                    }
                    widened = true;
                } finally {
                    await db.query(`
                        ...
                        ON DELETE ${widened ? 'SET NULL' : 'CASCADE'}
                    `);
                }
                console.log(widened
                    ? '[DB] Migration: order_items no longer cascades from products'
                    : '[DB] Migration: could not widen order_items.product_id; cascade left in place');
            }

            // Nothing to change on the key, but the column may still need widening (a
            // half-applied earlier run, or a schema created without the cascade).
            if (!cascading.length) {
                const productIdCol = cols.find(c => c.Field === 'product_id');
                if (productIdCol && String(productIdCol.Null).toUpperCase() === 'NO' && !fks.length) {
                    await db.query('ALTER TABLE order_items MODIFY product_id INT NULL');
                    console.log('[DB] Migration: order_items.product_id is nullable');
                }
            }
        } catch (err) {
            console.error('[DB] Error migrating order_items table:', err.message);
        }
