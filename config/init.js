...
const initDb = async () => {
    try {
        ...
        try {
            ...
            // guessed. Only the products key is touched; the one to orders is correct as
            // it is -- deleting an order should take its lines with it.
            ...
            for (const fk of fks) {
                if (String(fk.DELETE_RULE).toUpperCase() === 'CASCADE') {
                    ...
                    `);
                    console.log('[DB] Migration: order_items no longer cascades from products');
                }
            }
        } catch (err) {
