...
async function sync() {
    try {
        console.log('Syncing brand_type for initial brands...');

        for (const brand of initialBrands) {
            const bName = brand.name;
            const bType = brandCategories[bName] || 'All';

            // Update by name match (case insensitive-ish with LIKE)
            await db.query('UPDATE brands SET brand_type = ? WHERE name = ?', [bType, bName]);
            console.log(`Updated brand: ${bName} -> ${bType}`);
        }
